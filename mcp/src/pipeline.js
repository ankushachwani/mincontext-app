...
import { extractKeywords, contentMatchScore, pathMatchScore } from "../lib/keywords.js";
...
// ── Disk cache (24-hour TTL, persists across server restarts) ─────────────────
const CACHE_DIR = join(homedir(), ".cache", "mincontext");
const CACHE_FILE = join(CACHE_DIR, "mcp-cache.json");
...
function loadCache(source, task) {
  const store = loadDiskCache();
  const entry = store[`${source}::${task}`];
  if (!entry) return null;
  if (Date.now() > entry.expiresAt) return null;
  return entry.data;
}

function saveCache(source, task, data) {
  const store = loadDiskCache();
  ...
  store[`${source}::${task}`] = { data, expiresAt: now + CACHE_TTL };
  saveDiskCache(store);
}

// ── Head-scored content match ─────────────────────────────────────────────────
...
function detectRole(filePath, content) {
  ...
  if (ext === "py") {
    ...
    if (impls.length)
      roles.push(`[implements: ${impls.slice(0, 3).map((m) => `${m[1]} for ${m[2]}`).join(", ")}]`);
...
/**
 * Run the full mincontext pipeline for a GitHub repository.
 *
 * @param {string} task
 * @param {object} options
 * @param {string}   [options.owner]        - GitHub owner (required for GitHub source)
 * @param {string}   [options.repo]         - GitHub repo name (required for GitHub source)
 * @param {string}   [options.localDir]     - Absolute local path (required for local source)
 * @param {string}   [options.groqKey]      - Groq API key (falls back to GROQ_API_KEY env)
 ...
 * @returns {{ files: Array<{path,summary,reason}>, fromCache: boolean }}
 */
export async function runPipeline(task, options = {}) {
  ...
  if (!isLocal && (!owner || !repo)) throw new Error("Either localDir or owner+repo is required.");
  ...
  const contentMap = isLocal
    ? readLocalFiles(localDir, candidatePaths)
    : await fetchMultipleFiles(owner, repo, candidatePaths);
  ...
  if (keywords.length > 0 && allFilteredPaths.length > candidatePaths.length) {
    ...
    const validCut = allFilteredPaths.filter((p) => !selectedSet.has(p) && isRescuable(p));
    ...
    const coreSrc = validCut.filter(
      (p) => !kwSet.has(p) && p.split("/").some((d) => RESCUE_CORE_DIRS.has(d.toLowerCase()))
    );
  ...
  if (keywords.length > 0) {
    ...
    const high = withScores.filter((f) => f._cs >= CONTENT_THRESHOLD).sort((a, b) => b._cs - a._cs);
...
${fileDescriptions}`,
    ...
    if (usedFallback) ordered.slice(0, 6).forEach((f) => keptSet.add(f.path));
  }
  ...
  {"path":"app.go","type":"registration"}
    ...
    try {
      ...
      if (!checkResult.sufficient && Array.isArray(checkResult.missing)) {
        ...
        let added = 0, registrationAdded = 0;
